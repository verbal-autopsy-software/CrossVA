...
test_that("double selected: (selected(${isNeonatal}, '1') or selected(${isChild}, '1'))", {
    ## "(selected(${isNeonatal}, '1') or selected(${isChild}, '1'))"
...
test_that("multiple: ((selected(${isChild}, '1') and ${ageInMonthsByYear} >=48 ) or selected(${isAdult}, '1')) and (selected(${Id10051}, 'yes'))", {
    ## "((selected(${isChild}, '1') and ${ageInMonthsByYear} >=48 ) or selected(${isAdult}, '1')) and (selected(${Id10051}, 'yes'))"
...
test_that("multiple: selected(${isNeonatal}, '1') or (selected(${isChild}, '1') and (${ageInMonthsByYear} = 'NaN' or string-length(${ageInMonthsByYear}) = 0)) or (selected(${isChild}, '1') and ${ageInMonthsByYear} != 'NaN' and string-length(${ageInMonthsByYear}) > 0 and ${ageInMonthsByYear}<12)", {
    ## "selected(${isNeonatal}, '1') or (selected(${isChild}, '1') and (${ageInMonthsByYear} = 'NaN' or string-length(${ageInMonthsByYear}) = 0)) or 
    ##  (selected(${isChild}, '1') and ${ageInMonthsByYear} != 'NaN' and string-length(${ageInMonthsByYear}) > 0 and ${ageInMonthsByYear}<12)"
    relevant <- form151$relevant[465]
    newRelevant <- translate(relevant, death)
    expect_equal(newRelevant, "death$isNeonatal == 1 | (death$isChild == '1' & (is.na(death$ageInMonthsByYear) | death$ageInMonthsByYear == '') | (death$isChild == '1' & !is.na(death$ageInMonthsByYear) & death$ageInMonthsByYear & death$ageInMonthsByYear < 12")
})


test_that("double selected", {
    ## "not(selected(${Id10387}, 'yes')) and not(selected(${Id10388}, 'yes'))"
...
test_that("multiple: ((selected(${isAdult}, '1') and not(selected(${Id10310}, 'yes'))) and (${Id10019} = 'female') or ${Id10019}='undetermined')", {
    ## "((selected(${isAdult}, '1') and not(selected(${Id10310}, 'yes'))) and (${Id10019} = 'female') or ${Id10019}='undetermined')"
    relevant <- form151$relevant[437]
    newRelevant <- translate(relevant, death)
    expect_equal(newRelevant, "(death$isAdult == '1' and death$Id10310 != 'yes') & (death$Id10019 == 'female' | death$Id10019 == 'undetermined')")
})
